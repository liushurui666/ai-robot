...
2. 添加接收消息、读取单聊、发送机器人消息、读取资源和获取通讯录基本信息等权限，并把需要按姓名搜索的员工纳入应用通讯录权限范围。
...
当前 `.env` 可以复用 deploy-gateway 的模型与飞书凭据，代码和运行服务仍全部位于本项目。需要注意：飞书同一应用的事件订阅方式需要在“使用长连接接收事件”和“发送至开发者服务器”之间选择；如果该应用继续把事件回调给 deploy-gateway，本项目的长连接不会收到消息。正式并行运行时应给本项目单独创建飞书应用，或者明确把该应用的事件订阅切换到长连接。
...
本项目固定 `nanobot-ai==0.2.2`，镜像构建时会应用窄范围补丁：将飞书 SDK 已验证的发送者 ID 写入当前工具请求 metadata，并增加同一消息卡片的处理中/完成状态更新。补丁带源码结构校验；升级 nanobot 后若上游代码发生变化，构建会明确失败，必须先复核身份边界和卡片行为。
